...
Bun + TypeScript + Biome starter template. Ships **three modes** side by side; `bun run setup` keeps one and removes the rest:
...
Mode-specific: app has `start`/`dev`; lib has `build`/`dev` (tsdown) + `size` (size-limit); monorepo proxies `dev`/`build`/`test`/`typecheck` through `turbo run …` across all workspaces (root pins `packageManager` so Turbo can resolve them).
...
5. Monorepo mode only: `bun run build` (turbo) succeeds across all workspaces.
...
- Tests live in `tests/` next to the code (`src*/tests/*.test.ts`, or `<workspace>/tests/*.test.ts` in monorepo mode), using `bun:test`.
...
- `setup.ts` self-deletes after running; do not re-add `src-app`/`src-lib`/`src-monorepo` once a mode is chosen.
